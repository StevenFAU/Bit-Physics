# mypy: ignore-errors
"""Tests for common_warp.autodiff — Warp wp.Tape inverse-problem surface.

Mirrors common-py's test against a genuinely differentiable Warp forward
(linear model ``pred[i] = a·x[i] + b``): gradient cross-check, optimizer
convergence (adam/sgd/lbfgs), History recording, and the ParamSpec bridge.
"""
...
from common_warp.autodiff import (
    ControlProblem,
    GradientCheckReport,
    History,
...
def test_check_gradient_autodiff_matches_finite_difference():
    ...
    prob.set_target(_target_array(), device=_DEVICE)
