...
NOTE: the deliberately-broken `path:line` citations these tests feed to the
checker are assembled at RUNTIME via ``_cite`` so the literal backtick pattern
never appears in this source file — otherwise cat1's own intra-repo citation
scan would flag this test's fixture data as real broken citations. cat1 still
scans this file for any genuine citation mistakes in prose/comments.
"""
...
def _cite(path: str, start: int, end: int | None = None) -> str:
    """Build a backtick-fenced `path:line` (or `path:start-end`) citation.

    Assembled from parts so the literal pattern is absent from this source.
    """
...
def test_valid_in_range_citation_passes(tmp_path: Path) -> None:
    ...
    rel = _doc(tmp_path, "docs/d.md", f"see {_cite('tools/x.py', 3)} for detail\n")
    findings = run_cat4_path_line_assertions(tmp_path, [rel])
    assert findings == []


def test_missing_target_is_hard_fail(tmp_path: Path) -> None:
    rel = _doc(tmp_path, "docs/d.md", f"broken {_cite('tools/nope.py', 3)}\n")
    findings = run_cat4_path_line_assertions(tmp_path, [rel])
...
def test_out_of_range_line_is_hard_fail(tmp_path: Path) -> None:
    ...
    rel = _doc(tmp_path, "docs/d.md", f"see {_cite('tools/x.py', 9)}\n")
    findings = run_cat4_path_line_assertions(tmp_path, [rel])
    assert len(findings) == 1
...
def test_range_end_out_of_bounds_is_hard_fail(tmp_path: Path) -> None:
    ...
    rel = _doc(tmp_path, "docs/d.md", f"range {_cite('tools/x.py', 2, 9)}\n")
    findings = run_cat4_path_line_assertions(tmp_path, [rel])
    assert len(findings) == 1
    assert "out of range" in findings[0].message
...
def test_range_within_bounds_passes(tmp_path: Path) -> None:
    ...
    rel = _doc(tmp_path, "docs/d.md", f"range {_cite('tools/x.py', 2, 5)}\n")
...
def test_non_prose_suffix_is_out_of_scope(tmp_path: Path) -> None:
    ...
    rel = _doc(tmp_path, "docs/code.py", f"x = '{_cite('tools/nope.py', 9)}'\n")
    assert run_cat4_path_line_assertions(tmp_path, [rel]) == []
...
def test_file_outside_prose_roots_is_out_of_scope(tmp_path: Path) -> None:
    ...
    rel = _doc(tmp_path, "packages/foo/NOTES.md", f"{_cite('tools/nope.py', 9)}\n")
    assert run_cat4_path_line_assertions(tmp_path, [rel]) == []


def test_readme_root_is_in_scope(tmp_path: Path) -> None:
    """README.md at the repo root IS scanned (a named prose root)."""
    rel = _doc(tmp_path, "README.md", f"{_cite('tools/nope.py', 9)}\n")
    findings = run_cat4_path_line_assertions(tmp_path, [rel])
    assert len(findings) == 1
    assert "does not exist" in findings[0].message
...
def test_multiple_citations_on_one_line_each_flagged(tmp_path: Path) -> None:
    rel = _doc(tmp_path, "docs/d.md", f"{_cite('a/x.py', 1)} and {_cite('b/y.py', 2)}\n")
    findings = run_cat4_path_line_assertions(tmp_path, [rel])
...
def test_line_number_reported_is_the_prose_line(tmp_path: Path) -> None:
    rel = _doc(tmp_path, "docs/d.md", f"intro\n\n\nbad {_cite('tools/nope.py', 1)}\n")
    findings = run_cat4_path_line_assertions(tmp_path, [rel])
    assert len(findings) == 1
...
def test_files_none_scans_tracked_prose(tmp_path: Path) -> None:
    ...
    _doc(tmp_path, "docs/d.md", f"{_cite('tools/nope.py', 9)}\n")
...
def test_directory_citation_target_is_not_a_file(tmp_path: Path) -> None:
    ...
    rel = _doc(tmp_path, "docs/d.md", f"{_cite('tools/pkg', 1)}\n")
    findings = run_cat4_path_line_assertions(tmp_path, [rel])
    assert len(findings) == 1
    assert "does not exist" in findings[0].message
