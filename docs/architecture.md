...
> - **§ 2.13 (NEW; Phase-4 A3 honesty correction; Phase-4.1 first earned promotions)** — Mutation testing for testkit and integrity tooling. Per-target thresholds (80–95%); SOFT_WARN by default. HARD_FAIL-at-landing is an **earned per-target promotion**: the Phase-4.1 foundation-hardening pass earned the FIRST two — `render_similarity` (0.9242 ≥ 0.85) and `variant` (0.8702 ≥ 0.85), both lifted by oracle-grounded tests with no snapshot padding — which are now HARD_FAIL-at-landing. The remaining hardened targets (`common_3dgs` 0.7708; `code_verification_mms` 0.438-narrowed; `property` 0.6453-narrowed) stay SOFT_WARN advisory, recorded honestly below floor (not widened, not forced). See `docs/_audits/phase-4/foundation-hardening-2026-05-31T00-34-08Z.md`.
...
**Phase-4.1 earned promotions (the first per-target HARD_FAIL-at-landing targets).** The Phase-4.1 foundation-hardening pass added oracle-grounded constraining tests (published-constant anchors, cited-equation re-derivations, hand-computed values — no output snapshots) and re-measured each target live:

| Target | Before → After | Floor | Posture | Note |
|---|---|---|---|---|
| `render_similarity` (`metrics.py`) | 0.663 → **0.9242** | 0.85 | **HARD_FAIL-at-landing (PROMOTED)** | clears by +0.074; 16 residual all equivalent / error-string oos |
| `variant` (`equivalence/variant`) | 0.695 → **0.8702** | 0.85 | **HARD_FAIL-at-landing (PROMOTED)** | clears by +0.020; 17 residual all equivalent / oos |
| `common_3dgs` (`src`) | 0.663 → 0.7708 | 0.80 | SOFT_WARN advisory | +159 genuine kills; below floor (model-PLY / EWA / Warp-kernel lever) |
| `code_verification_mms` | 0.215 → 0.438 (narrowed) | 0.80 | SOFT_WARN advisory | class-(iii) narrowed (−327 cross-driven sim solutions); residual = report/HDF5/CLI glue |
| `property` | 0.170 → 0.6453 (narrowed) | 0.80 | SOFT_WARN advisory | class-(iii) narrowed (−656 test-files + per-sim); residual = testkit-core checker lever |

**Enforcement of the two promoted targets.** A phase-landing MUST re-measure the promoted targets and HARD_FAIL if either is below floor: `tools/testkit/mutation/run-mutation.sh --target render_similarity` (≥ 0.85) and `--target variant` (≥ 0.85). Both are small (≈ 211 + 131 mutants, ≈ 35 min combined), so a per-landing re-measure is tractable (unlike the full weekly T4 sweep). The committed real-scores ledger `tools/testkit/mutation/phase-4.1-hardening-*.json` records the earned scores; the `gate_helpers mutation-promoted-floor` check asserts every `posture: HARD_FAIL-at-landing` target in the latest ledger meets its floor (a regression that drops a promoted target below floor cannot land without updating — and thus re-justifying — the ledger). The advisory targets' narrowings (mms, property) exclude only source driven by OTHER runners (with dedicated targets) or test files; the de-duplications are recorded in `mutmut-config.toml` with written justification. Full classification + the surfaced follow-up levers: `docs/_audits/phase-4/foundation-hardening-2026-05-31T00-34-08Z.md`.
...
**Advisory sim/satellite targets.** Beyond the six spec-floored testkit/integrity modules above, `mutmut-config.toml` also carries additional per-sim and satellite targets (per-sim reference solvers and MMS solutions, the golden-table generators, etc.). These are **advisory, non-blocking** by default — they exist to accumulate a first real per-target baseline as each sub-phase lands, are not part of the §2.13 gate, and CI's mutation job excludes `packages/**`. A sub-phase may raise one to a gate by stating so in its plan (the Phase-4.1 hardening pass did exactly this — see the earned-promotions table above: `render_similarity` and `variant` were promoted out of advisory to HARD_FAIL-at-landing); absent that, they carry the 0.80 advisory floor inherited from the MMS solution target and never block a landing.
