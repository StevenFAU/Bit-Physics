...
No closed-form hand-derivation anchor is required: the update rule is
learned, not closed-form, so the golden anchor is the trained-checkpoint
L2 reconstruction.
...
- **Stack B (custom WGSL, local-only per spec § 7.8):**
  `packages/neural-ca/typescript/src/nca_inference.wgsl` — Stage 1b-B. The
  committed B-inference capture is generated by executing this WGSL on a GPU
  host. (The sibling `index.ts` driver stub was never implemented — the §0.3
  SHIFT below routed B-inference through wgsl_harness.py instead — and was
  deleted at the post-phase-5 housekeeping sweep, operator-ratified
  2026-06-10; the `.wgsl` file is the live Stack-B surface.)
...
(wgpu-native / Vulkan) — a genuine WGSL-on-GPU execution. (Pre-SHIFT prose
here said "the `index.ts` driver is the Phase-5 deploy path"; the LANDED
Phase-5 deploy path is `packages/neural-ca/web/src/main.ts`, which implements
its own pipeline and imports the same committed `nca_inference.wgsl` —
web-build track batch audits, 2026-06-09.)
...
1. **`golden_checkpoint_match`** (training golden, not cross-stack) — the
   ...
   bound, locked on measurement.
2. **§ 2.12 acceptance floors** — PSNR ≥ 28, SSIM ≥ 0.85, LPIPS ≤ 0.15.
3. **MEASURED D↔B render-similarity** (the locked gate; § 9).
...
`diagnostics.check_bounds` (RGBA ∈ [0, 1]).
