"""Property-based invariants for eulerian-smoke Stack-D (gate 11).

Ported from the Phase-1 reference invariants.py (spec-ref § 6.6); the algebra is
identical, so the invariants hold for the Stack-D Taichi ``project_pressure`` /
...
- :func:`divergence_free_post_projection` -- after one pressure-projection, the
  ...
  stencil floor; see ``stable_fluids_taichi.project_pressure``).
