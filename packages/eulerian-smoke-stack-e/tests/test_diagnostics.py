...
Mirrors the Phase-1 / Stack-D ``test_diagnostics`` but drives the Stack-E NVIDIA
Warp sim's ``compute_canonical_trajectory_3d`` (small N, in-memory; exercises
every kernel: trilinear SL advection, vorticity-confinement skeleton [eps=0 dead
path], viscous diffusion, fixed-20-sweep Jacobi projection, scalar density
advection):
...
The Stack-E sim module ``eulerian_smoke_stack_e.sim`` does NOT exist at the
...
def diagnostic_trajectory() -> _Trajectory:
    """In-memory diagnostic-tier Warp trajectory (small N, no I/O)."""
...
def test_tier1_health_no_nan_inf(diagnostic_trajectory: _Trajectory) -> None:
    """Diagnostic-tier Stack-E trajectory contains no NaN or Inf at any step."""
