"""Gate-4 code-verification: MMS observed-order-of-accuracy (Stack-E, MMS-ONLY).

Mirrors the Phase-1 / Stack-D ``test_mms_convergence`` but drives the NVIDIA Warp
Stack-E 2D ``stable_fluids_step`` (MacCormack-corrected semi-Lagrangian advect +
Jacobi pressure-projection) over the SHARED, byte-identical NS-2D Taylor-Green
forced manufactured solution at
...
Phase-1 NumPy reference observed advection 1.99 / projection 2.00; Stack-D
advection 1.9892 / projection 1.9976).

NOTE (Stage-1b precision; § L.6 O-W7 part-1): the 2D MMS arms multiply by ``0.25``
(exact in f32), so this MMS test is insensitive to the f64-literal trap; the 3D
Jacobi pure-literal normaliser ``1.0/6.0`` (seeded ``wp.float64(1.0)/wp.float64(6.0)``
-- Warp infers f32 absent the seed; Stage-0 R-A1 exercised it) is where the trap
surfaces, in the 3D capture (gate 14), not here.

The Stack-E reference submodule ``eulerian_smoke_stack_e.reference`` does NOT
exist at the failing-tests commit (Stage 1a) -- collection fails with
...
def test_mms_observed_ooa_advection_matches_formal() -> None:
    ...
    diag = (
        ...
        f"(eulerian-smoke-stack-e, t_final={_T_FINAL_ADVECTION}):\n"
        + "\n".join(ladder_lines)
...
def test_mms_observed_ooa_projection_matches_formal() -> None:
    ...
    diag = (
        "\nMMS projection convergence-rate ladder (eulerian-smoke-stack-e):\n"
        + "\n".join(ladder_lines)
        + f"\nobserved OOA = {observed_ooa:.4f}  "
        f"(formal = {_FORMAL_ORDER:.1f}, tolerance ±{_ORDER_TOLERANCE:.2f})"
    )
