...
@group(0) @binding(3) var<storage, read_write> kernelFields: array<vec2<f32>>;
@group(0) @binding(4) var<storage, read_write> affinityOut: array<vec4<f32>>;

@compute @workgroup_size(128)
fn perceive_growth(@builtin(global_invocation_id) g: vec3<u32>) {
  if (g.x >= U.n2) { return; }
  ...
  for (var kernel = 0u; kernel < 9u; kernel += 1u) {
    ...
    affinity[parameter.target_channel] += parameter.weight * growth;
  }
