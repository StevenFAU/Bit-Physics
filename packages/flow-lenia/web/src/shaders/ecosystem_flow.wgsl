...
@group(0) @binding(3) var<storage, read> kernelFields: array<vec2<f32>>;
@group(0) @binding(4) var<storage, read_write> transportOut: array<EcosystemTransportCell>;
@group(0) @binding(5) var<storage, read_write> diagnosticOut: array<FlowDiagnostic>;
...
fn compute_ecosystem_flow(@builtin(global_invocation_id) g: vec3<u32>) {
  ...
  var growth0 = vec4<f32>(0.0);
  var growth1 = vec4<f32>(0.0);
  var growth2 = vec4<f32>(0.0);
  for (var kernel = 0u; kernel < 9u; kernel += 1u) {
    let value = kernelFields[kernel * U.n2 + cell].y;
    if (kernel < 4u) { growth0[kernel] = value; }
    else if (kernel < 8u) { growth1[kernel - 4u] = value; }
    else { growth2[kernel - 8u] = value; }
  }
  transportOut[cell] = EcosystemTransportCell(mass, dx, dy, growth0, growth1, growth2);
