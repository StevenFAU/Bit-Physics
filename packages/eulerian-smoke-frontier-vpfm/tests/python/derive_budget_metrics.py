"""C-1 U-5 — derive REFRAMED-equivalence budget metrics from a canonical capture.

Charter § 3.5: the frontier-vs-parent equivalence is the REFRAMED metric-based gate
...
metrics). The U-4 script is the template; metrics are IDENTICAL by design so the
variant fixture is directly comparable against the SAME committed parent fixture
(probe § 4.5 single-source decision: U-5 cross-references the U-4 parent fixture).
...
writer transposes to match — vpfm.cpp put_field). Curl uses periodic central
