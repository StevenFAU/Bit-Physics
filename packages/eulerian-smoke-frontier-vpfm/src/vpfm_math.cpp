// C-1 U-5 stage 1b-i — analytic closed forms + deterministic host numerics
// (parallel-for, quadratic B-spline, periodic multigrid Poisson). U-4 substrate
// copy-adapt (probe § 2); derivations in vpfm_detail.hpp; the spec sheet § 2 records
// them as the hand-derivation anchor.
...
BSpline bspline_quadratic(double x_over_h) {
    ...
    s.dw[1] = -2.0 * r1;
    s.dw[2] = (1.5 + r2);
    // second derivative is the constant {1, -2, 1} stencil (detail header note).
    return s;
}
...
double poisson_periodic_mg(std::vector<double>& p, const std::vector<double>& rhs,
    ...
    // Compatibility: remove the mean of rhs so the periodic problem is solvable
    // (P2G vorticity is not exactly mean-free — the removed mean IS the total-
    // vorticity budget the diagnostics record); pin the solution mean to zero for
    // witness stability.
...
std::array<double, 3> taylor_green_vorticity(InitialCondition ic, double x, double y,
    ...
    // ω = ∇×u of the parent 3D TG (derivation in vpfm_detail.hpp header).
