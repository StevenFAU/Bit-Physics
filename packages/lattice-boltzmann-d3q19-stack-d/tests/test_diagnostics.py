"""Tier 1 + Tier 2 vector_field (IC-6) diagnostics tests (gates 6 + 7), Stack-D.

Mirrors the Phase-1 ``test_diagnostics`` against the Stack-D Taichi sim's
diagnostic-tier runner:

  - Tier 1: ``check_health`` (NaN/Inf scan on the diagnostic trajectory).
  - Tier 2 vector_field: ``check_divergence_free`` (advisory -- LBM is weakly
    compressible, so div(u) ~ 0 only at O(Ma^2)) + ``check_circulation``.

Uses ``sim_runner_diagnostic`` (coarse grid x few steps) -- exercises every
kernel (streaming, BGK collision, Guo forcing, bounce-back) without the
canonical-capture wall-clock + storage cost.
...
def test_tier2_vector_field_macroscopic_moments(tmp_path: Path) -> None:
    ...
    O(Ma^2). ``check_divergence_free`` records max|div(u)| as advisory; assert
    it stays below a generous bound. ``check_circulation`` (total integral of
    u_x) must stay finite.
    """
    manifest_path = sim_runner_diagnostic(seed=42, out_dir=tmp_path)
    h5_path = manifest_path.with_suffix(".h5")
    with h5py.File(h5_path, "r") as h:
