"""D3Q19 lattice constants (canonical ordering) -- Stack-D port.
...
ordering between Stack-D and the NumPy reference).

Pure data (no Taichi); the Stack-D Taichi kernels in ``d3q19_taichi`` consume
``C`` / ``W`` as f64 ndarrays. Sound speed + weights are first-principles
Gauss-Hermite values (Qian et al. 1992 section 2; Kruger 2017 Ch. 3 Table 3.4);
NOT re-derived at runtime.

Canonical-capture descriptors per docs/architecture.md Appendix D D.2.3:
