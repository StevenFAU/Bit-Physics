"""MPM PBT invariants (gate 11; spec section 6.6).

Two invariants port verbatim from the Phase-1 reference (spec section 6.6):

- :func:`mass_conservation_p2g_g2p` -- for any random particle IC, the P2G ->
  grid-identity round-trip preserves total particle mass (partition-of-unity).
- :func:`partition_of_unity_b_spline` -- for any particle position p, the sum of
  N(p - (base + k)) over the 3 neighbouring grid nodes equals 1.

Both are pure-Python entry points; ``mass_conservation_p2g_g2p`` dispatches to the
Taichi-DSL ``p2g`` kernel (cpu_max_num_threads=1; PBT scale, N <= 100 -- NOT
canonical-N).
"""
