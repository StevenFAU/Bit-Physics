...
The 2 invariants port verbatim from the Phase-1 MPM reference (spec 6.6):
...
matches the Phase-1 MPM + LBM Stage 1 precedent).

The Stack-D invariants module ``mpm_multimaterial_stack_d.invariants`` does
...
def test_mass_conservation_p2g_g2p(n_particles: int, seed: int) -> None:
    """sum m_p (initial) == sum_i grid_mass_i (after P2G).

    Hypothesis-driven: random particle counts (1..50) at random seeds.
    Grid is 16^3 with dx = 1/16; particles sampled uniformly in the
    interior ``[2*dx, 14*dx]`` so the 3x3x3 stencil stays in-bounds
...
def test_partition_of_unity_b_spline(p: float) -> None:
    """sum_{k in (0,1,2)} N(p - (base + k)) == 1 for any real p.
    ...
    assert s == pytest.approx(1.0, abs=1e-15), f"partition-of-unity violated at p={p}: sum={s}"
