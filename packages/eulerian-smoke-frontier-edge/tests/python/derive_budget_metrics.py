"""C-1 U-6 — derive REFRAMED-equivalence budget metrics from a canonical capture.

Charter § 3.6: the frontier-vs-parent equivalence is the REFRAMED metric-based gate
(vortical-agreement budget trajectories), NOT pointwise comparison — the parent's own
cross-stack record (equivalence.md § 2) measured Lyapunov growth λ≈0.12–0.29/step on
this very descriptor, and the parent canonical trajectory is MEASURED blown up by its
first captured interval (the U-4 1c finding; fixture clebsch-pfm-equivalence/
parent-budget-metrics.json).
...
metrics). The U-4/U-5 script is the template; metrics are IDENTICAL by design so the
EDGE variant fixture is directly comparable against the SAME committed parent fixture
(probe § 4.5 single-source decision, carried from U-5: EDGE cross-references the U-4
parent fixture — derive_budget_metrics.py computes byte-identical metric definitions
for every flow-map unit, so no second parent capture is run).

Usage (from tools/testkit, the U-3/U-5 ctest cwd precedent):
    uv run --no-sync python <this file> <capture_manifest.json> <out_metrics.json>

Axis convention: capture fields are [x][y][z] (axis 0 = x; parent layout, the EDGE
writer transposes to match — edge.cpp put_field). Curl uses periodic central
