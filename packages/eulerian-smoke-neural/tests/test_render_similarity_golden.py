...
Renders the canonical 3dgs-smoke sim and compares each frame to the project's OWN committed
golden renders (``tools/testkit/golden/renders/eulerian-smoke-neural-canonical-frame-*``) via the
``render_similarity`` harness. DETERMINISTIC own-pipeline regression (the smoke step + coupling +
common-3dgs rasterizer are all bit-exact/same-hw): the metrics MUST clear the §2.12 floors
(PSNR >= 28 / SSIM >= 0.85 / LPIPS <= 0.15). Below-floor = STOP-RENDER-FLOOR.

RED at Stage 1a (the sim raises ``NotImplementedError`` + goldens not yet committed); GREEN at 1b.
"""
...
def test_golden_renders_exist(golden_renders_dir: Path) -> None:
    frames = sorted(golden_renders_dir.glob("eulerian-smoke-neural-canonical-frame-*.png"))
...
def test_render_similarity_clears_floors(
    ...
    checked = 0
    for fr in frames:
        golden_path = golden_renders_dir / f"eulerian-smoke-neural-canonical-frame-{fr.step}.png"
