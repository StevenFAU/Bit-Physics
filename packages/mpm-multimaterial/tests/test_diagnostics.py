...
pattern; conventions doc § M inheritance). Per probe report § 2:
...
MPM is the **first sub-phase to consume BOTH IC-5 AND IC-6** (plan
§ 1.2 ➂).

Use ``sim_runner_diagnostic`` (16³ × 5K particles × 50 steps) —
...
def test_tier2_particle_momentum_conservation(tmp_path: Path) -> None:  # type: ignore[no-untyped-def]
    ...
    stays finite and below a conservative upper bound (10×
    ...
    with h5py.File(h5_path, "r") as h:
        ...
        assert drift < 1.0, (
            ...
            f"bound 1.0 (drop-impact with gravity; expected drift << 1 over 50 steps × 1e-4 dt)"
        )


def test_tier2_vector_field_grid_momentum(tmp_path: Path) -> None:  # type: ignore[no-untyped-def]
    """IC-6 ``check_circulation_grid_mom_l1`` — grid momentum field is finite + bounded.

    Per spec § 6.3 / probe report § 2: the grid momentum field is
    derived (P2G of particle momentum) rather than primitively stored;
    the L1 norm (volume-weighted) is a finite proxy for the
    circulation diagnostic at this sub-phase.
    """
    manifest_path = sim_runner_diagnostic(seed=42, out_dir=tmp_path)
    h5_path = manifest_path.with_suffix(".h5")
    with h5py.File(h5_path, "r") as h:
        ...
        assert mom_l1 >= 0.0, (
            f"check_circulation_grid_mom_l1 = {mom_l1:.3e} negative (L1 norm)"
        )
        # Upper bound: 100.0 in volume-weighted momentum units (well above
        # expected ~5K particle × ~1e-4 mass × ~2 m/s velocity × ~unit volume).
        assert mom_l1 < 100.0, (
