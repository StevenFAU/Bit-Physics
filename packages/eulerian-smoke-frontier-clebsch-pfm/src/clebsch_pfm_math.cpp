// C-1 U-4 stage 1b-i — analytic closed forms + deterministic host numerics
// (parallel-for, quadratic B-spline, periodic multigrid Poisson). Derivations in
// clebsch_pfm_detail.hpp; the spec sheet § 2 records them as the hand-derivation
// anchor.
...
BSpline bspline_quadratic(double x_over_h) {
    ...
    // node offsets relative to x: r0 = fx+1, r1 = fx, r2 = fx-1
    double r0 = fx + 1.0, r1 = fx, r2 = fx - 1.0;
    s.w[0] = 0.5 * (1.5 - r0) * (1.5 - r0);
    s.w[1] = 0.75 - r1 * r1;
    s.w[2] = 0.5 * (1.5 + r2) * (1.5 + r2);
    // d w / d x_over_h (chain through r = x_over_h - node)
    s.dw[0] = -(1.5 - r0);
    s.dw[1] = -2.0 * r1;
    s.dw[2] = (1.5 + r2);
    return s;
...
// Colour-parallel: every update of a colour reads only the opposite colour.
    ...
    parallel_for(static_cast<std::size_t>(nf) * nf * nf,
                 [&](std::size_t lo, std::size_t hi) {
                     for (std::size_t c = lo; c < hi; ++c) {
                         ...
                         // fine cell centre in coarse index space (cell-centred grids)
...
double poisson_periodic_mg(std::vector<double>& p, const std::vector<double>& rhs,
    ...
    // Compatibility: remove the (roundoff-scale) mean of rhs so the periodic problem
    // is solvable; pin the solution mean to zero for witness stability.
...
Spinor taylor_green_wave_2d(double x, double y, double hbar) {
    const double k = 2.0 * M_PI;
    // z = λ/2 = −cos(2πx); θ = 4μ/ħ with μ = −cos(2πy)/(2π)  (derivation in detail.hpp)
    double zc = -std::cos(k * x);
    double theta = 4.0 * (-std::cos(k * y) / k) / hbar;
    double ca = std::sqrt(0.5 * (1.0 + zc));  // cos(α/2), α ∈ [0, π]
    double sa = std::sqrt(0.5 * (1.0 - zc));  // sin(α/2)
...
double wave_velocity_face(const Spinor& a, const Spinor& b, double hbar, double dx) {
    ...
    double im = (a[0] * b[1] - a[1] * b[0]) + (a[2] * b[3] - a[3] * b[2]);
    return (hbar / dx) * std::atan2(im, re);
}
