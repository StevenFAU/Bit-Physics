...
def test_capture_has_gradient_fields(tmp_path: Path) -> None:
    ...
    assert len(gfs) == 1
    gf = gfs[0]
    assert gf["name"] == "dLoss_dalpha"
    assert gf["wrt"] == "alpha"
    assert gf["dtype"] == "float64"
    assert gf["shape"] == [1]
    assert manifest_path.with_suffix(".h5").exists()
    assert manifest["sim"]["name"] == "neural-ca"
    assert manifest["sim"]["variant"] == "frontier-difflogic"
    assert manifest["sim"]["category"] == "continuous-ca"
...
def test_capture_roundtrips(tmp_path: Path) -> None:
    ...
    assert "final_state" in step0.fields
    assert "dLoss_dalpha" in step0.fields
